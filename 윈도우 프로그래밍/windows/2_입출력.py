...
def click():
  ...
  # Text에 문자 출력하기(END는 문장의 끝가지 삭제 후 문자 삽입)
...
# 버튼을 눌러 출력하기
tk.Button(root, text="확인", command=click, width = 10, height=2).grid(row=1, columnspan=2) #칼럼스팬=셀병합
...
def submit():
  ...
  try:
    ...
    output.delete(0.0, END) #입력상자 초기화
    output.insert(END, "단어를 찾을 수 없습니다") #단어 삽입

root = Tk()
root.title("컴퓨터 용어 사전")
root.option_add("*font", "돋움 13")

Label(root, text="검색할 단어를 입력해 주세요").grid(row=0, column=0, sticky=W)
